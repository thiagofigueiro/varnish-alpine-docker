...
class TestInstagram(VarnishTest):
    def test_200_cached(self):
        assert 200 == self.get_response_code(
            '/static/images/homepage/screenshot1-2x.jpg/2debbd5aaab8.jpg',
            allow_redirects=True,
        )
        sleep(1)
        assert 0 < self.get_age(
            '/static/images/homepage/screenshot1-2x.jpg/2debbd5aaab8.jpg')


class TestGoogle(VarnishTest):
    def test_302(self):
        assert 302 == self.get_response_code('/images')
